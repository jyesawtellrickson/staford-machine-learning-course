...
% output h is a 5000 by 10 matrix where each row represents 
% a training example with each column a probability of a category
h = a3;

n = size(a3)(2);

% y represents the labelled solutions, need to convert to 0 and 1s
% iterate across classes
for i = 1:n
	
	y_mat = y==i;
	J = J + (-y_mat'*log(h(:,i))-(1-y_mat)'*log(1-h(:,i)));
